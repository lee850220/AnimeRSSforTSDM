...
function CHECK_INTERNET {
    resp=$(curl -m 15 -s google.com >& /dev/null; echo $?)
    if [ $resp -ne 0 ]; then
...
[/table]\""
response=$(curl -m 15 -sX POST "https://www.tsdm39.net/forum.php?mod=post&action=newthread&fid=${FID}&topicsubmit=yes" --header "${TSDM_Cookie}" --form "formhash=${FORMHASH}" --form "typeid=${TYPE}" --form "subject=${pmtitle}[${filesize}]" --form 'usesig="1"' --form 'allownoticeauthor="1"' --form 'addfeed="1"' --form 'wysiwyg="0"' --form "message=[align=center][b]This is a post generated by script wrote by Inanity緋雪, 
if you find any problem please contact me ASAP, thanks.[/align][/b]
[table=98%]
[tr][td]鏈接[/td][td=90%] [url=${LINK}]${LINK}[/url][/td][/tr]
[tr][td]提取碼[/td][td]${sharePW}[/td][/tr]
...
RCUP
curl -m 15 -sX POST "https://www.tsdm39.net/forum.php?mod=post&action=reply&fid=${FID}&tid=${TID}&replysubmit=yes" --header "${TSDM_Cookie}" --form "formhash=${FORMHASH}" --form "typeid=${TYPE}" --form 'usesig="1"' --form "message=${post}"; echo
RCDOWN
...
curl -m 15 -sX POST ${LINE_API} --header 'Content-Type: application/x-www-form-urlencoded' --header "Authorization: Bearer ${LINE_TOKEN}" --data-urlencode \
