...
function RCUP {
    ...
    while [ $BREAK -eq 0 ]; do
        if [ -e ${MUTEX} ]; then
            echo "${Notice}${pmtitle} Enter Critical Section... Failed. sleep 5 sec."
            sleep 5
        else
            touch ${MUTEX}
            echo "${Notice}${pmtitle} Enter Critical Section... Success."
            BREAK=1
        fi
    done
...
function CSHARE {
    ...
    do
        ...
        if [ "$chk" = "0" ]; then
            ...
            echo "Creating share link of ${fileID}... Success."
            ...
            if [ "$resp" = "" ]; then
                echo "${Notice}Creating share link of ${fileID}... Failed. (reason: Empty response.)"
            else
                resp=$(echo $resp| grep -o "show_msg[^}]*" | tr -d "\"" | sed 's/show_msg://')
                echo "${Notice}Creating share link of ${fileID}... Failed. (reason: ${resp})"
            fi
...
"Message=     Task failed. (reason: TSDM post reply failed)
[Task]：      ${NEW}")
...
# Save record file
if [ ! -d "${RecordDIR}/${EPISODE_NAME}" ]; then
    mkdir "${RecordDIR}/${EPISODE_NAME}"
fi
touch "${RecordDIR}/${EPISODE_NAME}/${filename_ext}.posted"
